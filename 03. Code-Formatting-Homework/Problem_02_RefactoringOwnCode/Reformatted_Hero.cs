...
namespace HeroesOfFate.Models.Characters.Heroes
{
    public abstract class Hero : Character, IInventory
    {
        ...
        public double ArmorReduce
        {
            get { return this.armorReduce; }
            set
            {
                if (value <= 0)
                {
                    throw new ArgumentOutOfRangeException(string.Format(ExceptionConstants.NullOrNegativeException, "Armor reduce value"));
                }
                this.armorReduce = value; 
            }
        } 
